...
# ClassPinoutList 
#   manages list of possible pinouts and holds 
#   filename of current pinout json file.
# PinoutParser: provides parsing of pinout json files.
# Subclasses implement specific parsers:
#   class GPIO_MIDI --> stores pin definitions and MIDI notes
#   class SaveNewPinout --> Receives new json from web page
...
class PinoutParser:
    ...
    def parse( self, json_data ):
        ...
        actions = {
            ...
            "neopixel": lambda pin:  self.define_neopixel(toi(pin)), # (gpio number)
            "tachometer": lambda pin: self.define_tachometer(toi(pin)), # (gpio number)
            "microphone": lambda pin: self.define_microphone(toi(pin)), # (gpio number)
            "touchpad": lambda pin: self.define_touchpad(toi(pin)), # (gpio number)
            "gpio.midi": lambda pin, instrument, midi_num, rank: self.define_gpio_midi( toi(pin), midi.Note( toi(instrument), toi(midi_num) ), rank ),
            "i2c": lambda sda, scl: self.define_i2c( toi(sda), toi(scl) ), # (sda, scl)
            "mcp23017": lambda addr: self.define_mcp23017( toi(addr)),  # (address)
            "mcp.midi": lambda pin, instrument, midi_num, rank: self.define_mcp_midi( toi(pin), midi.Note( toi(instrument), toi(midi_num) ), rank ),
        }
        for pd in json_data:
            actions[pd[0]]( *pd[1:]) 
...
# Singleton objecto to hold GPIO pin definitions and general pinout info
...
class SaveNewPinout(PinoutParser):
    ...
    def _add_MIDI_to_list( self, midi_note ):
        self._add_to_list( midi_note, self.midilist, f"Duplicate MIDI note {midi_note}")
    ...
    def define_mcp23017( self, address ):
        ...
        self._add_to_list( f"{self.current_I2C}.{address}", self.mcplist, f"Duplicate MCP, I2C {self.current_I2C} address {address}")

    def define_mcp_midi( self, mcp_pin,  midi_note, rank ):
        self._add_MIDI_to_list( midi_note )
        mp = f"{self.current_I2C}.{self.current_MCP_address}.{mcp_pin}"
        self._add_to_list( mp, self.mcp_port_list, f"Duplicate MIDI port I2C.MCPaddress.port={mp}" )
...
class PinoutList:
    ...
    def get_description( self, filename=None ):
        ...
        if self.pinout_files[filename] == "":
            ...
            self.pinout_files[filename] = PinoutParser( filename ).get_description()
