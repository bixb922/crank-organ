...
class Setlist:
        ...
        while True:
            ...
            if len( self.current_setlist ) == 0:
                if self.touch_button.is_double_touch():
                    ...
                    self.logger.info(f"Automatic play, shuffle all tunes {len(self.current_setlist)}")
            ...
            self.player_task = asyncio.create_task( player.play_tune( tuneid ) )
            try:
            ...
            self.logger.info(f"play_tune ended" )
    ...
    def save( self ):
        ...
        fileops.write_json( self.current_setlist, config.SETLIST_JSON, keep_backup=False )
    ...
    def to_beginning_of_tune( self ):
        ...
        tuneid = player.get_progress()["tune"]
        if tuneid:
            self.stop_tune()
